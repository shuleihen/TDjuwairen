...
- (void)requestWithData{
    NSString *urlPath = @"http://192.168.191.1/tuanda_web/Appapi/index.php/View/view_show1_2/id/44";
    NetworkManager *ma = [[NetworkManager alloc] init];
    [ma GET:urlPath parameters:nil completion:^(id data, NSError *error){
        if (!error) {
            self.dataDic = data;
            
            NSString *urlpath = [NSString stringWithFormat:@"http://192.168.191.1%@",self.dataDic[@"view_content_url"]];
...
- (void)setupWithTableView{
    self.tableview = [[UITableView alloc]initWithFrame:CGRectMake(0, 1, kScreenWidth, kScreenHeight-64-64) style:UITableViewStyleGrouped];
...
{
//    NSLog(@"%@",self.sfview.fontArr[indexPath]);
