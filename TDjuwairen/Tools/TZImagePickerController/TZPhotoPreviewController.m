...
- (void)viewWillAppear:(BOOL)animated {
    ...
    _titleLabel.text = [NSString stringWithFormat:@"%ld / %ld",_currentIndex+1,self.models.count];
...
- (void)viewDidLayoutSubviews {
    ...
    _titleLabel.frame = CGRectMake((self.view.tz_width-120)/2, 10, 120, 44);
...
- (void)select:(UIButton *)selectButton {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            NSString *title = [NSString stringWithFormat:@"最多选择 %ld张图片", _tzImagePickerVc.maxImagesCount];
...
- (void)doneButtonClick {
    ...
    if (_progress > 0 && _progress < 1 && (_selectButton.isSelected || !_tzImagePickerVc.selectedModels.count )) {
        _alertView = [_tzImagePickerVc showAlertWithTitle:@"同步iCloud中的图片"];
        return;
    }
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    _numberLabel.hidden = (_tzImagePickerVc.selectedModels.count <= 0 || _isHideNaviBar);
